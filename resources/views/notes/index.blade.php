            ...
            <div class="card note" style="background-color: {{ $note->color }}">
                <div class="card-body p-0">
                        ...
                        <div class="d-flex justify-content-end">
                            <a href="{{ url('notes/changePriority', $note->id) }}">
                                @switch($note->priority)
                                    @case(0)
                                        <i class="p-3 note__icon far fa-star"></i>
                                        @break

                                    @case(1)
                                        <i class="p-3 note__icon fas fa-star"></i>
                                        @break
                                    @default
                                @endswitch
                            <a href="{{ url('notes', $note->id) }}"><i class="p-3 fas fa-pencil-alt note__icon"></i></a>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-between">
                        <i class="p-3 fas fa-trash-alt note__icon deleteModal" data-toggle="modal" data-target="#deleteModal" data-id="{{ $note->id }}" data-title="{{ $note->title }}" data-delete-link="{{ route('notes.destroy', $note->id) }}"></i>
                        <i class="p-3 fas fa-palette note__icon colorModal" data-toggle="modal" data-target="#colorModal" data-id="{{ $note->id }}" data-title="{{ $note->title }}" data-color="{{ $note->color }}" data-color-link="{{ url('notes/changeColor', $note->id) }}"></i>
